...
def get_all_uploaded_csv_names(path):
...
def get_dataframe_from_csv_by_table_name(argument, common_argument, common_argument_for_batch):
    table = argument
    path = common_argument['override_path']
    return {
        'table': table,
        'dataframe': get_dataframe_from_path(f"{path}/{table}.csv")
    }

def load_all_dataframes(path):
    all_dataframes = {}
    all_tables = get_all_uploaded_csv_names(path)

    results = parallel.execute(
        func=get_dataframe_from_csv_by_table_name,
        argument_list=all_tables,
        common_arguments={'override_path': path}
        )
